<div>
    <div class="flex items-center mb-8">
        ...
        <h2 class="text-3xl font-extrabold text-rose-600">{{ $warehouse->name }} - odpad</h2>
    </div>
        ...
        <x-card class="shadow-xl mb-8">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="flex flex-col">
                    ...
                    <button wire:click="setInput('amount')"
                            class="w-full text-center py-4 text-4xl font-black rounded border-4 transition-all duration-200 {{ $currentInput === 'amount' ? 'border-rose-500 bg-rose-50 text-rose-700 shadow-inner' : 'border-gray-100 bg-gray-50 text-gray-400 hover:border-gray-200' }}">
                        {{ $amount }}
                    </button>
                ...
                <div class="flex flex-col">
                    <x-label value="Cena" />
                    <x-select wire:model.live="priceLevelId" class="w-full text-center py-4 text-2xl font-black border-4 border-gray-100 bg-gray-50 text-gray-700">
                        @forelse($priceLevels as $pl)
                            <option value="{{ $pl->id }}">{{ $pl->price }} Kč ({{ $pl->amount }} {{ $product->unit }})</option>
                        @empty
                            <option disabled>Není skladem</option>
                        @endforelse
                    </x-select>
                </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <button wire:click="submit()"
                        class="col-span-2 flex items-center justify-center bg-rose-600 text-white rounded shadow-lg hover:bg-rose-700 active:transform active:scale-95 transition-all duration-150 disabled:opacity-50"
                    ...
                    @else
                        ...
                        <span class="ml-2 text-xl font-bold uppercase tracking-widest">Uložit do odpadu</span>
                    @endif
                </button>
            </div>
        </x-card>
